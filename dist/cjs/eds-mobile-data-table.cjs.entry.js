...
const ENOVOSMobileDataTable = class {
    ...
    this.renderValue = value => {
      if (value.props) {
        switch (value.props.type) {
          case 'button':
            return (index.h("eds-button", { "margin-bottom-1": true, styles: value.props.styles || 'secondary', "text-only": !!value.props.textOnly, onClickButton: value.props.onClickButton }, index.h("eds-icon", { icon: value.props.icon, slot: "icon" })));
          case 'html':
            return value;
          default:
            return index.h("eds-paragraph", { type: "body-1", content: value.data });
        }
      }
      return index.h("eds-paragraph", { type: "body-1", content: value.data });
    };
  ...
  render() {
    return (index.h("div", { class: this.getComponentClassName() }, this.groups.map(group => index.h("div", { class: `${this.classNames.EL}__group` }, group.map(item => index.h("span", { class: this.getItemClassName(item.value) }, item.title &&
      index.h("eds-paragraph", { "font-weight": "bold", type: "body-2", content: item.title }), this.renderValue(item.value)))))));
  }
  get el() { return index.getElement(this); }
};
ENOVOSMobileDataTable.style = mobileDataTableCss;

exports.eds_mobile_data_table = ENOVOSMobileDataTable;
