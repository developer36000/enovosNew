...
class IntersectionObserverController {
  ...
  constructor(idleTimeout = 50, trackHovers = false) {
    ...
    this._mutationsObserver = undefined;
  ...
  scheduleUpdate(timestamp) {
    const calledFromRAF = typeof timestamp === 'number';
...
class IntersectionObservation {
  ...
  updateIntersection(root, rootRect) {
    const targetRect = getRectangle(this.target);
    const intersection = this.getIntersectionData(root, rootRect, targetRect);
    let threshold = +intersection.exists;
    const ratioChanged = intersection.ratio !== this.prevRatio;
    const targetRectChanged = !isEqual(targetRect, this.prevTargetRect);
    let thresholdChanged;
    ...
    if (thresholdChanged) {
      const entry = new IntersectionObserverEntry(this.target, targetRect, intersection.rect, intersection.ratio, rootRect, now());
  ...
  getIntersectionData(container, containterRect, targetRect) {
    ...
    const detached = isDetached(container, target);
    const intersecRect = !detached ? getIntersection(container, target, containterRect, targetRect) : emptyRect;
    const intersects = !detached && intersecRect.width >= 0 && intersecRect.height >= 0;
    const intersecRatio = getArea(intersecRect) / getArea(targetRect) || 0;
    return {
      rect: intersecRect,
      ratio: intersecRatio,
      exists: intersects,
 ...
 */
function parseMargins(marginsInput = '0px') {
  ...
  const margins = (marginsInput + '').split(/\s+/);
...
class IntersectionObserver {
   ...
   */
  constructor(callback, options = { threshold: 0, rootMargin: '', root: undefined }, controller, publicObserver) {
  ...
  getThresholdGreaterThan(ratio) {
    ...
    const thresholdsLen = thresholds.length;
    let index = 0;
   ...
   */
  getRootRect() {
    const rootRect = getRectangle(this._root);
...
class IsVisibleObservable {
  ...
  initIntersectionObserver() {
    if (this.element) {
      this.observer = new IntersectionObserver$1((entries) => {
...
exports.IsVisibleObservable = IsVisibleObservable;
