...
const ENOVOSCard = class {
  constructor(hostRef) {
    ...
    this.clickCard = index.createEvent(this, "clickCard", 7);
  ...
  async componentDidRender() {
    ...
    if (cardMedia) {
      ...
      cardMediaClass = cardMediaClass.concat(ComponentPropsHelper.ComponentPropsHelper.setGlobalStyles(this.backgroundMedia, this.classNames.EL).split(' '));
      cardMedia.className = '';
      cardMedia.classList.add(...compact.compact(cardMediaClass));
    }
    if (cardMediaContent) {
      const cardMediaContentClass = [`${this.classNames.EL}${this.classNames.MEDIA}${this.classNames.CONTENT}`];
      cardMediaContent.classList.add(...compact.compact(cardMediaContentClass));
    }
    if (cardMediaTopLeft) {
      const cardMediaTopLeftClass = [`${this.classNames.EL}${this.classNames.MEDIA}${this.classNames.TOP}${this.classNames.LEFT}`];
      cardMediaTopLeft.classList.add(...compact.compact(cardMediaTopLeftClass));
    }
    if (cardMediaTopRight) {
      ...
      cardMediaTopRight.classList.add(...compact.compact(cardMediaTopRightClass));
    }
    if (cardMediaBottomLeft) {
      ...
      cardMediaBottomLeft.classList.add(...compact.compact(cardMediaBottomLeftClass));
    }
    if (cardMediaBottomRight) {
      ...
      cardMediaBottomRight.classList.add(...compact.compact(cardMediaBottomRightClass));
    }
    ...
    if (cardContent) {
      ...
      cardContentClass = cardContentClass.concat(ComponentPropsHelper.ComponentPropsHelper.setGlobalStyles(this.backgroundContent, this.classNames.EL).split(' '));
      cardContent.className = '';
      cardContent.classList.add(...compact.compact(cardContentClass));
    }
  ...
  render() {
    return (index.h("div", { class: [
      ...
      ].join(' '), onClick: () => (this.clickable) ? this.clickCard.emit() : false, style: (this.backgroundImage) ? { 'background-image': `url("${this.backgroundImage}")` } : {} }, index.h("div", { class: `${this.classNames.EL}${this.classNames.MEDIA}${this.classNames.CONTAINER}` }, index.h("slot", { name: "card-media" }), index.h("slot", { name: "card-media-content" }), index.h("slot", { name: "card-media-top-left" }), index.h("slot", { name: "card-media-top-right" }), index.h("slot", { name: "card-media-bottom-left" }), index.h("slot", { name: "card-media-bottom-right" })), index.h("slot", { name: "card-content" }), (this.video)
      ? index.h("eds-video", { type: "mp4", class: [
...
const ENOVOSVideo = class {
  ...
  async componentDidLoad() {
    ...
    if (this.videoAutoplay === true) {
      this._videoObservable = new isVisibleObservable.IsVisibleObservable(this.el, async (value) => {
        if (value === true) {
          await this.play();
  ...
  async componentDidRender() {
    ...
    if (this.playOnClick === true) {
      this._clickHandler = new tapEvent.TapEvent(this.el, async () => {
        await this.play();
      });
    }
  }
  ...
  /** REMOVABLE START */
  render() {
    return (index.h("video", { height: this.height, width: this.width, poster: this.posterPath, preload: this.preload, class: this.getClasses(), muted: true, playsinline: true }, index.h("source", { src: this.path, type: this.getType() })));
  }
  get el() { return index.getElement(this); }
};
ENOVOSVideo.style = videoCss;

exports.eds_card = ENOVOSCard;
exports.eds_video = ENOVOSVideo;
