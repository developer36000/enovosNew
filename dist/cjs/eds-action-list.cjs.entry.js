...
const ENOVOSActionList = class {
  constructor(hostRef) {
    ...
    this.clickActionItem = index.createEvent(this, "clickActionItem", 7);
    this.fitted = false;
  ...
  setShape() {
    ...
    if (itemActiveEl && shapeEl) {
      ...
      const indexItem = findIndex.findIndex(this.actionItems, (item) => item.getProp('id') === this.activeAction);
  ...
  getCurrentActionConfiguration(propKey) {
    const activeItem = find.find(this.actionItems, (item) => item.getProp('id') === this.activeAction);
    return (activeItem)
  ...
  render() {
    return (index.h("div", { class: [
      ...
      switch (item.getProp('type')) {
        case 'button':
          return index.h("div", { "data-id": `${item.getProp('id')}`, "data-nb": this.actionItems.length, class: `${this.classNames.EL}${this.classNames.ITEM}${this.classNames.CONTAINER}` }, index.h("eds-button", { class: `${this.classNames.EL}${this.classNames.ITEM}`, size: this.size, expand: this.fitted === false ? true : false, outlined: this.getActionConfiguration(item, 'outlined'), textOnly: this.getActionConfiguration(item, 'textOnly') || (this.segmented && this.activeAction !== item.getProp('id')), content: item.getProp('content'), styles: this.getActionConfiguration(item, 'styles'), disabled: item.getProp('disabled') === true ? true : false, "icon-position": item.hasProp('iconPosition') ? item.getProp('iconPosition') : false }, item.hasProp('icon')
            ? index.h("eds-icon", { slot: "icon", icon: item.getProp('icon') })
            : ''), item.hasProp('badge')
            ? index.h("eds-badge", { class: `${this.classNames.EL}${this.classNames.ITEM}${this.classNames.BADGE}`, text: item.getProp('badge.text'), size: item.hasProp('badge.size') ? item.getProp('badge.size') : 'md', styles: item.hasProp('badge.styles') ? item.getProp('badge.styles') : 'success' })
            : '');
      }
    })));
  }
...
exports.eds_action_list = ENOVOSActionList;
